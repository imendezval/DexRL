...
def main():
    ...
    mesh_folder = args_cli.input
    if not os.path.isabs(mesh_folder):
        mesh_folder = os.path.abspath(mesh_folder)

    # create destination path
    dest_folder = args_cli.output
    ...
    # Physics Properties
    mass_props      = schemas_cfg.MassPropertiesCfg(mass=args_cli.mass) if args_cli.mass else None
    rigid_props     = schemas_cfg.RigidBodyPropertiesCfg()              if args_cli.mass else None   
    ...
    for obj_file in os.scandir(mesh_folder):
        ...
        stem = os.path.splitext(obj_file.name)[0]        
        dest_path = os.path.join(dest_folder, f"{stem}.usd")
        ...
        print("-" * 80)
        print("-" * 80)
        print(f"Input Mesh file: {mesh_folder}")
